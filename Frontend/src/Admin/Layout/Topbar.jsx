...
const Topbar = ({ title}) => {
    ...
    const handleImageChange = async (e) => {
      ...
      try {
        const res = await axios.put(
          "http://localhost:5000/auth/updated-avatar", // apna backend URL daal
          formData,
          {
            headers: {
  ...
  return (
    <div className="bg-white shadow-md px-8 py-4 flex justify-between items-center rounded-b-lg">
      ...
      <p className="text-sm text-black mt-1 ">welcome :
        <span className="font-medium text-gray-700"> {user?.fullName} </span> </p>
      </div>
