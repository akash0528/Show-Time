...
import {AiOutlinePlus,AiOutlineEdit,AiOutlineDelete,AiOutlineSearch,AiOutlineFileText} from "react-icons/ai";
...
const  AdminArtist = () => {
  ...
  const fetchArtist = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/admin/artist",{
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are You sure to Delete!")) {
      try {
        await axios.delete(`http://localhost:5000/api/admin/artist/${id}`,{
          withCredentials:true
        });
  ...
  const handleFeatureToggle = async (id, value) => {
    try {
      ...
      await axios.patch(
        `http://localhost:5000/api/admin/home/artist/feature/${id}`,
        { isFeatured: value },
        { withCredentials: true }
      )
         ...
         <div className="w-16 h-24 rounded-2xl overflow-hidden shrink-0 shadow-2xl border border-white/10">
          ...
          <div className="flex-1">
           ...
           <div className="flex items-center gap-2">
            ...
            onChange={(e) => handleFeatureToggle(m._id, e.target.checked)}/>
