...
const EventSlider = () => {
  ...
  const fetchAllData = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/api/home/feature`)
          ...
          <SwiperSlide key={event._id}>
            <div className="flex  md:flex-row items-center justify-between bg-linear-to-r from-purple-400 via-pink-500 to-red-500 text-white px-4 md:px-12 py-10 gap-8 ">
              ...
              <div className="md:w-1/2 space-y-3 md:space-y-4 text-center md:text-left md:ml-20 ">
                <h2 className="md:text-4xl text-xl font-bold text-gray-800">{event.title}</h2>
                <p className="text-gray-800 text-sm md:text-2xl ">{event?.date && formatDate(event.date)} {event?.timing}</p>
                <p className="text-gray-700 font-medium text-sm md:text-xl">{event.location}</p>
                <p className="text-sm md:text-xl font-semibold text-blue-700">₹{event.price}</p>
